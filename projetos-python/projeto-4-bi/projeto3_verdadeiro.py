...
def verifica_notas(nome_materia):
    while True:
        try:
            ...
            print("Nota inválida! Por favor, insira um valor numérico")
...
def forms_digitacao():
    try:
        while True:
            ...
            while len(aluno_ra) > 6 or not aluno_ra.isnumeric() or aluno_ra == "" or verificar_matricula(aluno_ra):
                if verificar_matricula(aluno_ra):
                    print("Matrícula já cadastrada! Insira uma matrícula única.")
...
def menu():
    ...
    print("2 - Ver alunos do Excel;")
    print("3 - Sair.")
    opc = input("Escolha uma das opções entre 1 e 3: ")
    while not opc.isnumeric() or int(opc) < 1 or int(opc) > 3:
        opc = input("Escolha uma das opções entre 1 e 3 apenas: ")
